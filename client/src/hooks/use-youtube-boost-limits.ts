...
// ───── Plan Tiers ─────
export type YoutubePlan = 'free' | 'basic' | 'pro' | 'premium';

const LIMITS: Record<YoutubePlan, Record<string, number>> = {
...
const STORAGE_KEY = 'boostify_youtube_usage';
const BANNER_TIMER_KEY = 'boostify_youtube_banner_shown';
...
function getPlanFromSubscription(sub: string | null | undefined, isAdmin: boolean): YoutubePlan {
  if (isAdmin) return 'premium';
  if (!sub) return 'free';
  const s = sub.toLowerCase();
  if (s === 'premium' || s === 'enterprise') return 'premium';
  if (s === 'pro' || s === 'professional' || s === 'youtube_boost_pro') return 'pro';
  if (s === 'basic' || s === 'creator') return 'basic';
  return 'free';
}

export function useYoutubeBoostLimits() {
