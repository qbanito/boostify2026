...
/**
 * Ticket Messenger — send tickets, reminders and VIP upgrades over WhatsApp.
 * Uses the message/media gateway with ticket-oriented templates.
 */
export function TicketMessenger({ center }: { center: WhatsAppCenter }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <StatTile label="Ingresos WhatsApp" value={`$${(center.analytics?.revenue ?? 0).toFixed(0)}`} accent="amber" />
        <StatTile label="Fans activos" value={center.analytics?.activeFans ?? 0} accent="sky" />
      </div>
      <GlassCard className="p-4">
        <div className="mb-3 flex items-center gap-2 text-sm text-white/70">
          <QrCode className="h-4 w-4 text-emerald-300" /> Plantilla de ticket (edítala antes de enviar)
        </div>
