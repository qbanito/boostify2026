 ...
 * transactions, and confirm internal conversions over WhatsApp.
 */
export function BTFWalletMessenger({ center }: { center: WhatsAppCenter }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <StatTile label="Ingresos WhatsApp" value={`$${revenue.toFixed(2)}`} accent="amber" />
      ...
      <GlassCard className="flex items-center gap-2 p-3 text-xs text-white/55">
        <ArrowDownUp className="h-4 w-4 shrink-0 text-emerald-300" />
