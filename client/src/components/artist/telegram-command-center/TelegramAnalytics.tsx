...
const INTENT_LABEL: Record<string, string> = {
  ...
  start_live: 'Lives', create_community: 'Comunidades', launch_release: 'Lanzamientos',
  greeting: 'Saludos', opt_out: 'Opt-out', unknown: 'Otros',
};

/** Analytics — operational KPIs for the Telegram channel. */
export function TelegramAnalytics({ center }: { center: TelegramCenter }) {
  ...
  return (
    <div className="space-y-5">
      <PanelHeader
        ...
        title="Analytics"
        subtitle="Mensajes, fans activos, conversión e ingresos generados por Telegram."
      />
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTile label="Ingresos" value={`$${(a?.revenue ?? 0).toFixed(0)}`} accent="sky" />
        <StatTile label="Tickets" value={a?.ticketsSold ?? 0} accent="rose" />
        <StatTile label="Comunidades" value={a?.communitiesCount ?? 0} accent="sky" />
      ...
      <GlassCard className="p-4">
        <h4 className="mb-4 flex items-center gap-2 text-sm font-semibold text-white/80">
          <TrendingUp className="h-4 w-4 text-sky-300" /> Comandos más usados
        </h4>
