...
/** Connection status dot — blue when connected, red otherwise. */
export function StatusDot({ connected }: { connected: boolean }) {
  return (
    <span className="relative inline-flex h-2.5 w-2.5">
      ...
      <span className={`relative inline-flex h-2.5 w-2.5 rounded-full ${connected ? 'bg-sky-400' : 'bg-rose-500'}`} />
    </span>
  );
}
...
export function StatTile({ label, value, accent = 'sky' }: { label: string; value: ReactNode; accent?: string }) {
  return (
    <GlassCard className="p-4">
      <div className={`text-2xl font-bold ${ACCENT_TEXT[accent] || ACCENT_TEXT.sky}`}>{value}</div>
...
/**
 * ConciergeComposer — reusable "send a message to one subscriber" form used by
 * the Tickets, Merch, Booking, Live Gifts and Wallet concierge panels.
 */
export function ConciergeComposer({
  center, placeholder, defaultMessage = '', ctaLabel = 'Enviar por Telegram',
}: {
  center: TelegramCenter; placeholder: string; defaultMessage?: string; ctaLabel?: string;
}) {
  const [chatId, setChatId] = useState('');
  const [message, setMessage] = useState(defaultMessage);
  const [mediaUrl, setMediaUrl] = useState('');
  const disabled = !center.isConnected || !chatId.trim() || !message.trim();

  const onSend = () => {
    if (disabled) return;
    if (mediaUrl.trim()) {
      center.sendMedia.mutate({ chatId: chatId.trim(), mediaUrl: mediaUrl.trim(), caption: message.trim() });
    } else {
      center.sendMessage.mutate({ chatId: chatId.trim(), message: message.trim() });
    }
  };
  ...
  return (
    <div className="space-y-3">
      <input
        ...
        placeholder="Chat ID del fan (ej. 123456789)"
      ...
      <motion.button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 px-4 py-3 text-sm font-semibold text-white transition disabled:opacity-40"
      >
        {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
        {ctaLabel}
      </motion.button>
      {!center.isConnected && (
        <p className="text-center text-xs text-amber-300/80">Conecta tu bot de Telegram para enviar mensajes.</p>
      )}
    </div>
  );
}
