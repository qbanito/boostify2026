...
import { Users, Star, ShoppingBag, MapPin, UserPlus, Send, Loader2, Upload, AtSign } from 'lucide-react';
...
/**
 * Fan Community — browse subscribers by segment, import a list and send a 1:1
 * message. Subscribers grow automatically when fans message your bot (/start).
 * Consent + opt-out are enforced server-side.
 */
export function FanCommunity({ center }: { center: TelegramCenter }) {
  ...
  const send = () => {
    ...
    center.sendMessage.mutate({ chatId: selected, message: message.trim() });
    setMessage('');
  };

  const doImport = () => {
    // Parse simple "name,chatId,city,vip" lines.
    const contacts = csv.split('\n').map((line) => {
      const [name, chatId, city, vip] = line.split(',').map((x) => x?.trim());
      if (!chatId) return null;
      return { name: name || chatId, chatId, city: city || null, isVip: /^(1|true|vip|si|sí)$/i.test(vip || ''), source: 'import', consentStatus: 'opted_in' as const };
  ...
  return (
    <div className="space-y-5">
      <PanelHeader
        ...
        title="Fan Community"
        subtitle="Segmenta tus suscriptores y conversa 1:1. Crecen solos cuando escriben /start a tu bot."
      />
      ...
      {showImport && (
        <GlassCard className="p-4">
          <p className="mb-2 text-xs text-white/60">Pega suscriptores (uno por línea): <code>nombre,chatId,ciudad,vip</code></p>
          <textarea
            value={csv}
            onChange={(e) => setCsv(e.target.value)}
            rows={4}
            placeholder={'Ana,123456789,CDMX,vip\nLuis,987654321,Guadalajara,'}
          ...
          <button
            ...
            className="mt-2 flex items-center gap-2 rounded-xl bg-sky-500/90 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40"
          >
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <GlassCard className="max-h-80 overflow-y-auto p-2">
          {filtered.length === 0 ? (
            <p className="p-6 text-center text-sm text-white/40">Aún no hay suscriptores en este segmento.</p>
          ) : (
            filtered.map((c) => (
              <button
                key={c.id}
                ...
                className={`flex w-full items-center justify-between gap-2 rounded-xl px-3 py-2.5 text-left transition ${
                  selected === c.chatId ? 'bg-sky-500/15' : 'hover:bg-white/5'
                }`}
              >
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-white/45">
                    {c.username ? <span className="flex items-center gap-0.5"><AtSign className="h-3 w-3" />{c.username}</span> : <span>{c.chatId}</span>}
        ...
        <GlassCard className="p-4">
          ...
          <motion.button
            ...
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40"
          >
