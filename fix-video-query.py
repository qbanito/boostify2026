...
# The broken section: unclosed if block and old fallback-only logic
old_fragment = (
    "          } catch (e) {\n"
    "          logger.warn('\u00e2\u009a\xa0\u00ef\u00b8\x8f Error searching videos by pgId:', e);\n"
    "        }\n"
    "        \n"
    ...
    "          } catch (e) {\n"
    "            logger.warn('\u00e2\u009a\xa0\u00ef\u00b8\x8f Error searching videos by Firebase UID:', e);\n"
    "          }\n"
    "        }"
)
...
if old_fragment in content:
    ...
    with open('client/src/components/artist/artist-profile-card.tsx', 'w', encoding='utf-8') as f:
        f.write(content)
    print("SUCCESS: Video query updated")
else:
