/**
 * Script para extraer y mostrar datos de artistas generados en Firestore
 * Permite visualizar la información completa o filtrada de los artistas
 */
import * as admin from 'firebase-admin';
import { readFileSync } from 'fs';
import * as path from 'path';

// Inicializar Firebase Admin de forma alternativa
try {
  // Primero intentamos inicializar con las credenciales predeterminadas
  if (admin.apps.length === 0) {
    admin.initializeApp({
      projectId: 'artist-boost',
    });
    console.log('Firebase Admin inicializado con configuración predeterminada');
  }
} catch (error) {
...
/**
 * Extrae todos los artistas generados de Firestore
 * @param limit Número máximo de artistas a extraer (opcional)
 * @returns Array de artistas
 */
async function extractGeneratedArtists(limit?: number) {
  try {
    console.log('Extrayendo artistas generados de Firestore...');
    
    let query = db.collection('generated-artists');
...
async function main() {
  ...
  // Buscar límite de artistas
  const limitArg = args.find(arg => arg.startsWith('--limit='));
  const limit = limitArg ? parseInt(limitArg.split('=')[1]) : undefined;
  
  // Extraer artistas
  const artists = await extractGeneratedArtists(limit);
